...
const Home = () => {
    ...
    const loadMovies = async () => {
        ...
        try {
            ...
            // XỬ LÝ AN TOÀN: Tìm đúng mảng dữ liệu trả về từ API
            let actualData = [];
            if (Array.isArray(res.data)) {
                ...
                actualData = res.data.content; // Trường hợp dùng Spring Data Page
            ...
            setIsEnd(actualData.length < 20); // 20 là kích thước trang mặc định của bạn
        } catch (ex) {
            console.error("Lỗi tải phim:", ex);
            setMovies([]); // Reset về mảng rỗng để tránh lỗi map()
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="home-container">
            ...
            <div className="filter-section">
                ...
                <div className="filter-controls">
                    ...
                    <div className="filter-group price-group">
                        ...
                        <input type="range" min="0" max="150000" className="price-slider" />
                    </div>
            ...
            {/* HIỂN THỊ PHIM TRỐNG (Chỉ hiển thị khi đã load xong và mảng thực sự rỗng) */}
                    ...
                    <Col xs={12} sm={6} md={4} lg={3} key={p.id}>
                        <Card className="movie-card-custom h-100" onClick={() => nav(`/movies/${p.id}`)}>
                            ...
                            <Card.Body className="d-flex flex-column info-container">
                                <Card.Title className="movie-title">{p.movieName}</Card.Title>
                                <div className="tags-container">
                                    <span className="tag">{p.category ? p.category : "Đang cập nhật"}</span>
                                </div>
                                <div className="bottom-meta mt-auto">
                                    <span className="duration">Thời lượng: {p.duration} phút</span>
                                </div>
                            </Card.Body>
                        </Card>
                    </Col>
